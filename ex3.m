...
for nnnz = 3:3 % 6:-1:3
    ...
    LLLLz = 1.0;
    LLLLx = 2*LLLLz;
    coord = gs_coord_rectangle(nnnx,nnnz,LLLLx,LLLLz);
    if nnnz == 3
        elem = gs_elem(coord,3,0);
    elseif nnnz == 4
        elem = gs_elem(coord,12,0); % ATT on overlap !!!!!
    elseif nnnz == 5
        elem = gs_elem(coord,3,0);
    elseif nnnz == 6
        elem = gs_elem(coord,15,0);
    end
...
% <SUPPORT>
    ...
    kind = [];
    for i = 1:nn
        if coord(i,1) == 0 && coord(i,3) == 0 
            support_coord = [support_coord; coord(i,:)];
            kind = [kind; "xyz"];
        end
        if coord(i,1) == LLLLx && coord(i,3) == 0 
            support_coord = [support_coord; coord(i,:)];
            kind = [kind; "yz"];
        end
        support_coord = [support_coord; coord(i,:)];
...
% <LOAD>
    ...
    kk = 1;
    for i = 1:nn
        if (0 < coord(i,1) && coord(i,1) < LLLLx) && (coord(i,3) == 0)
...
% <MASS_MATRIX>
    ...
    for i = 1:nn
        if ((0 < coord(i,1) && coord(i,1) < LLLLx) && coord(i,3) == 0) || (0 < coord(i,3))
